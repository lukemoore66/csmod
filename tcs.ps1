...
If ((Get-Location).Path -ne $PSScriptRoot) {Throw ("This script can only be run from it's own directory: " + $PSScriptRoot)}
...
If (!(Test-Path -LiteralPath $INIPathDefault)) {Throw ("Default ini file '" + $INIPathDefault + "' does not exist.")}
...
Get-Content -LiteralPath $InputListPath | % {
	If ($_ -and $_.Trim()) {
		...
		If (Test-Path -LiteralPath $arrLine[0]) {$objInputList.Add($arrLine[0], (Get-INIPath $arrLine[0] $arrLine[1]))}
		#Otherwise The Input File Does Not Exist
		Else {Write-Warning ("Input file: '" + $arrLine[0] + "' at line " + $intLineCounter + " in " + $InputListPath + " is invalid, skipping.")}
	}
}
...
If ($objInputList.Count -lt 1) {Throw ("Input list: '" + $InputListPath + "' is empty.")}
...
$objInputList.GetEnumerator() | % {
	...
	Add-Content -LiteralPath $OutputLogPath -Value $strInputListLine
